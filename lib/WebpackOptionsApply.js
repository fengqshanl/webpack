...
class WebpackOptionsApply extends OptionsApply {
	...
	process(options, compiler) {
		...
		// compile ExternalsPlugin tab 定义
		// 上述钩子调用时传入值 { normalModuleFactory } 定义 factorize ExternalModuleFactoryPlugin tabAsync 钩子 (normalModuleFactory.hooks.factorize.tapAsync)
		...
		// 跟上面 挂载了 同样的 钩子， 不过这次没有在 compile 上面挂载，只是在 normalModuleFactory.hooks.factorize.tapAsync 排除了 node-commonjs 的相关插件
		...
		// compiler.hooks.compilation.tap                             =>  ChunkPrefetchPreloadPlugin
		// compilation.hooks.additionalChunkRuntimeRequirements.tap		=>  ChunkPrefetchPreloadPlugin
		// 									.additionalTreeRuntimeRequirements.tap		=>  ChunkPrefetchPreloadPlugin
		// 									.runtimeRequirementInTree.tap							=>  ChunkPrefetchPreloadPlugin
		new ChunkPrefetchPreloadPlugin().apply(compiler);

		if (typeof options.output.chunkFormat === "string") {
			switch (options.output.chunkFormat) {
				case "array-push": {
					// compiler.thisCompilation.tap    											=> ArrayPushCallbackChunkFormatPlugin
					// compilation.additionalChunkRuntimeRequirements.tap   => ArrayPushCallbackChunkFormatPlugin
					//						.renderChunk.tap													=> ArrayPushCallbackChunkFormatPlugin
					// 						.chunkHash.tap														=> ArrayPushCallbackChunkFormatPlugin
					const ArrayPushCallbackChunkFormatPlugin = require("./javascript/ArrayPushCallbackChunkFormatPlugin");
					new ArrayPushCallbackChunkFormatPlugin().apply(compiler);
					break;
				}
				case "commonjs": {
					// compiler.thisCompilation.tap   											=> CommonJsChunkFormatPlugin
					// compilation.additionalChunkRuntimeRequirements.tap 	=> CommonJsChunkLoadingPlugin
					// 						.renderChunk.tap 													=> CommonJsChunkFormatPlugin
					// 						.chunkHash.tap														=> CommonJsChunkFormatPlugin
					const CommonJsChunkFormatPlugin = require("./javascript/CommonJsChunkFormatPlugin");
					new CommonJsChunkFormatPlugin().apply(compiler);
					break;
				}
				case "module": {
					// compiler.thisCompilation.tap   											=> ModuleChunkFormatPlugin
					// compilation.additionalChunkRuntimeRequirements.tap 	=> ModuleChunkFormatPlugin
					// 						.renderChunk.tap 													=> ModuleChunkFormatPlugin
					// 						.chunkHash.tap														=> ModuleChunkFormatPlugin
		...
		/**
		 *  compiler.hooks.thisCompilation.tap   											=> Jsonp / ImportScripts / CommonJs / Module ...  ChunkLoadingPlugin
		 * 	compilation.hooks.runtimeRequirementInTree   							=> Jsonp / ImportScripts / CommonJs / Module ...  ChunkLoadingPlugin
		 */
		...
		/**
		 * 	compiler.hooks.thisCompilation.tap    										=> FetchCompile / ReadFile / ReadFileCompileAsync ... WasmPlugin
		 * 	compilation.hooks.runtimeRequirementInTree								=> FetchCompile / ReadFile / ReadFileCompileAsync ... WasmPlugin
		 */
		...
		/**
		 * 	compiler.hooks.thisCompilation.tap												-> AbstractLibraryPlugin
		...
		/**
		 * 	compiler.hooks.compilation.tap														=> ModuleInfoHeaderPlugin
		 * 	compilation.hooks.renderModulePackage											=> ModuleInfoHeaderPlugin
		 * 									 .chunkHash																=> ModuleInfoHeaderPlugin
		 */
		...
		/**
		 * 	compiler.hooks.emit.tapAsync 															=> CleanPlugin
		 * 	compilation.hooks.keep
		 */
		...
		/**
		 * 	compiler.hooks.compilation.tap														=> EvalSourceMapDevToolPlugin
		 * 	conpilation.hooks.renderModuleContent											=> EvalSourceMapDevToolPlugin
		 * 									 .inlineInRuntimeBailout									=> EvalDevToolModulePlugin
		 * 									 .render																	=> EvalDevToolModulePlugin
		 * 									 .chunkHash																=> EvalDevToolModulePlugin
		 * 									 .additionalModuleRuntimeRequirements			=> EvalDevToolModulePlugin
		 */
		if (options.devtool) {
